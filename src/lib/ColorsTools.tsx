...
function guessColorCategory(value: string): string {
  ...
  if (hue !== null && hue !== undefined) { // When hue is not undefined or null
...
type CalculateHarmoniousColors = (h: number, s: number, l: number, a: number, name: string) => CalculatedColor[];
...
const calculateHarmoniousColors: CalculateHarmoniousColors = (h, s, l, a, name) => {
  ...
  const accent: CalculatedColor = {
    ...
    a,
    name: "accent",
    colorName: guessColorCategory(`hsla(${h}, ${Math.min(s + 10, 100)}%, ${Math.min(l + 10, 100)}%, ${a})`),
  ...
  const primaryForeground: CalculatedColor = calculateForegroundColor(primary.l, "primary-foreground");
  const secondaryForeground: CalculatedColor = calculateForegroundColor(secondary.l, "secondary-foreground");
  const tertiaryForeground: CalculatedColor = calculateForegroundColor(tertiary.l, "tertiary-foreground");
  ...
  const cardPrimaryForeground: CalculatedColor = calculateForegroundColor(cardPrimary.l, "primary-card-foreground");
  const cardSecondaryForeground: CalculatedColor = calculateForegroundColor(cardSecondary.l, "secondary-card-foreground");
  const cardTertiaryForeground: CalculatedColor = calculateForegroundColor(cardTertiary.l, "tertiary-card-foreground");

  return [
    primary, secondary, tertiary, accent, success, error,
    cardPrimary, cardSecondary, cardTertiary,
    cardPrimaryForeground, cardSecondaryForeground, cardTertiaryForeground,
...
export { createDynamicStyle, guessColorCategory, adjustHue , calculateHarmoniousColors, calculateForegroundColor, calculateCardColor};