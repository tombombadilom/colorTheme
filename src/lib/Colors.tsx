...
const PickColor: FC<Props> = ({ color, index, handleNewColor }: Props): JSX.Element => {
  ...
  const [hslaColor, setHslaColor] = useState<string>(`hsla(${hue}, ${saturation}%, ${lightness}%, ${alpha})`);
  useEffect(() => {
  ...
  const handleHueChange = (e: ChangeEvent<HTMLInputElement>) => setHue(parseInt(e.currentTarget.value, 10));
  const handleSaturationChange = (e: ChangeEvent<HTMLInputElement>) => setSaturation(parseInt(e.currentTarget.value, 10));
  const handleLightnessChange = (e: ChangeEvent<HTMLInputElement>) => setLightness(parseInt(e.currentTarget.value, 10));
  const handleAlphaChange = (e: ChangeEvent<HTMLInputElement>) => setAlpha(parseFloat(e.currentTarget.value));
  ...
  return (
    <Card key={"c" + index}
      ...
      >
        {show && 
          <div>
            <HslaColorPicker
              ...
              onChange={(newColor) => NewColor({
                ...
                a: newColor.a,
                name: color.name,
                colorName: color.colorName || guessColorCategory(`hsla(${newColor.h}, ${newColor.s}%, ${newColor.l}%, ${newColor.a})`),
          ...
          />
            ...
            />
              <Input placeholder="Hue" type="range" id="hue" name="hue" min="0" max="360" value={hue} onChange={handleHueChange} />
              <Input placeholder="Saturation" type="range" id="saturation" name="saturation" min="0" max="100" value={saturation} onChange={handleSaturationChange} />
              <Input placeholder="Lightness" type="range" id="lightness" name="lightness" min="0" max="100" value={lightness} onChange={handleLightnessChange} />
              <Input placeholder="Alpha" type="range" id="alpha" name="alpha" min="0" max={1} step={0.01} value={alpha} onChange={handleAlphaChange} />
          </div>
      ...
      >
        <Button
          className="flex-1"
          onClick={() => toggleShow(!show)}>{!show ? "Set Color" : "Hide Color Picker"}</Button>
        <Button
          className="flex-1"
          onClick={() => handleNewColor({ name: color.name, h: hue, s: saturation, l: lightness, a: alpha, value: hslaColor })}>Save</Button>
        <Button
          className="flex-1"
        >Reset</Button>
      </CardFooter>
...
const Colors: FC = (): JSX.Element => {
  const [colors, updateColors] = useState([
    { name: "primary", colorName: "blue", h: 240, s: 100, l: 50, a: 1, value: "hsla(240,100%,50%,1)" },
    { name: "secondary", colorName: "purple", h: 300, s: 100, l: 50, a: 1, value: "hsla(300,100%,50%,1)" },
    { name: "tertiary", colorName: "pink", h: 350, s: 100, l: 50, a: 1, value: "hsla(350,100%,50%,1)" },
  ]);
  ...
  const handleNewColor: HandleNewColor = (color) => {
    ...
    const newColorsArray: CalculatedColor[] = colors.map(({ name: colorName, h, s, l, a, value }) => ({
      name,
      colorName,
      h,
      s,
  ...
  return (
    <>
      {colors.map((color, index) => PickColor({ color, index, handleNewColor }))}
    </>
  )
}
