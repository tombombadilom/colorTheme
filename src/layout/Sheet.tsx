...
const MySheet: React.FC<SheetProps> = ({ children }) => {
	...
	return (
		<Suspense fallback={<div>Loading...</div>}>
			<Sheet open={isSheetOpen} onOpenChange={() => toggleSheet()} key={side}>
				<Drawer key={"bottom"} open={isDrawerOpen} onOpenChange={toggleDrawer}>
					<Header
						colorTheme={
							<DrawerTrigger
								onClick={(e) => {
				...
				>
					<SheetHeader>
						<SheetTitle>
							...
							/>
							<p className="mt-6 text-center text-gray-300 text-1xl">
								ColorTheme
							</p>
						</SheetTitle>
						<SheetDescription>
