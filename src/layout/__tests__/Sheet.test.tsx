...
describe("MySheet", () => {
	...
	it("toggles Sheet when the Menu is clicked", async () => {
		...
		const { getByRole, queryByRole } = render(
			<MySheet>{dummyChildren}</MySheet>,
		);
		const menuButton = getByRole("button", { name: /menu/i });

		fireEvent.click(menuButton);
		await waitFor(() =>
			expect(queryByRole("dialog", { name: /sheetcontent/i })).toBeVisible(),
		);

		fireEvent.click(menuButton);
		await waitFor(() =>
			expect(
				queryByRole("dialog", { name: /sheetcontent/i }),
			).not.toBeVisible(),
		);
	});
	...
	it("toggles Drawer when the Palette is clicked", async () => {
		...
		await waitFor(() =>
			expect(queryByRole("dialog", { name: /drawercontent/i })).toBeVisible(),
		);

		fireEvent.click(paletteButton);
		await waitFor(() =>
			expect(
				queryByRole("dialog", { name: /drawercontent/i }),
			).not.toBeVisible(),
		);
	});
